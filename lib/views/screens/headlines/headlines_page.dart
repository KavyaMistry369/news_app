  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<Pagecontroller>(
      builder: (context,p,_) {
        return Scaffold(
          appBar: AppBar(
            leading: IconButton(onPressed: (){}, icon: Icon(Icons.drag_handle)),
            actions: [
                      ...
                      actions: [
                        Column(
                          ...
                          children: [
                            RadioListTile(
                                title: Text("India"),
                                value: selectCountry.IN, groupValue: p.select, onChanged: (selectCountry? v){
                                p.setCountry(val: v!);
                                HeadlineHelper.headlineHelper.countryHeadlinesNews(select: v.name);
                                }),
                            RadioListTile(
                                title: Text("America"),
                                value: selectCountry.US, groupValue: p.select, onChanged: (selectCountry? v){
                              p.setCountry(val: v!);
                              HeadlineHelper.headlineHelper.countryHeadlinesNews(select: v.name);
                            }),
                            RadioListTile(
                                title: Text("Canada"),
                                value: selectCountry.CA, groupValue: p.select, onChanged: (selectCountry? v){
                              p.setCountry(val: v!);
                              HeadlineHelper.headlineHelper.countryHeadlinesNews(select: v.name);
                            }),
                            RadioListTile(
                                title: Text("China"),
                                value: selectCountry.CN, groupValue: p.select, onChanged: (selectCountry? v){
                              p.setCountry(val: v!);
                              HeadlineHelper.headlineHelper.countryHeadlinesNews(select: v.name);
                            }),
                            RadioListTile(
                                title: Text("Russia"),
                                value: selectCountry.RU, groupValue: p.select, onChanged: (selectCountry? v){
                              p.setCountry(val: v!);
                              HeadlineHelper.headlineHelper.countryHeadlinesNews(select: v.name);
                            }),
                          ],
